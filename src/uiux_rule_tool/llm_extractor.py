...
def _build_instructions() -> str:
    return (
        "你是一个 UI/UX 规范规则抽取器。"
        "请从输入内容中提炼结构化规则，请确保输入内容里所有规范相关的内容都有对应的规则，并遵守以下要求："
        ...
        "15. 如果文档里有大等于3条同类型的颜色值、像素值、百分比等罗列值一起出现，且没有涉及用途、使用场景、适用场景，这通常表示该项的可选枚举值；你需要把这些同类型原始值汇总为一条“只能从这些枚举值中选择其一”的规则，并保留完整原始枚举值。"
